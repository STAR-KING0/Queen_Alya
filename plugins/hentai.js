...
smd(
  ...
  async (m) => {
    try {
      ...
      const apiUrl = `https://api.giftedtechnexus.co.ke/api/search/xnxxsearch?query=${encodeURIComponent(query)}&apikey=gifteddevskk`;
...
smd({
  ...
  try {
    ...
    const apiUrl = `https://api.giftedtechnexus.co.ke/api/download/xnxxdl?url=${encodeURIComponent(videoUrl)}&apikey=gifteddevskk`;
